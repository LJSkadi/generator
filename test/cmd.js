...
describe('elgenerate(1)', function () {
  ...
  describe('(no args)', function () {
    ...
    it('should provide debug instructions', function () {
      assert.ok(/DEBUG=elgenerate\(1\)-\(no-args\):\* (?:\& )?npm start/.test(ctx.stdout))
    });
  ...
  describe('(unknown args)', function () {
    ...
    it('should print usage', function (done) {
      runRaw(ctx.dir, ['--foo'], function (err, code, stdout, stderr) {
        if (err) return done(err);
        assert.ok(/Usage: elgenerate/.test(stdout));
